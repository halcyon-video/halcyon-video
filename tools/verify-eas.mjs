import {checkEasLifecycle} from './check-eas-lifecycle.mjs';
...
const port=6220,html=resolve(root,'tools/verify-eas.html');
...
await page.goto(`http://localhost:${port}/tools/verify-eas.html`);await page.waitForFunction(()=>!!window.boot);
await page.evaluate((label)=>{localStorage.clear();Object.entries({bb_store_format:'corporate',bb_theme:'bb-2000',bb_quality:'low',bb_ssao:'0',bb_tv_demo_loop:'0',bb_outside:'day'}).forEach(([k,v])=>localStorage.setItem(k,v));},label);
if(label==='lifecycle') {
writeFileSync(resolve(out,'lifecycle.json'),JSON.stringify(await checkEasLifecycle(page),null,2));
} else {
console.log('Booting store');await page.evaluate(()=>window.boot());console.log('Store ready');
await delay(2500);
if(label==='before' && await page.evaluate(()=>!!window.store.scene.getObjectByName('eas-pedestal-model'))) throw Error('Before photographs require the original implementation');
if(label!=='before') await page.waitForFunction(()=>window.store.scene.getObjectByName('eas-pedestal-model'));
const anchor=await page.evaluate(()=>{const v=window.store.entrance.getVestibuleInfo();return [v.xL-.62,0,v.sideDoorZ];});
const views=[['pair',[-4,4.8,-5],[0,1.5,0]],['side',[1,2.5,-4],[0,1.6,-1.85]],['rear',[-.8,1.0,3.4],[0,.45,2.15]]];
...
if(label!=='before') writeFileSync(resolve(out,'lifecycle.json'),JSON.stringify(await checkEasLifecycle(page),null,2));
