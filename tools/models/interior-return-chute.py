...
cut('Open rear counter socket',-1.46,1.46,.08,3.46,-1.60,FACE-.14)
cut('Left through aperture',-1.4,-.4,2.4,2.7,.18,FACE+.08)
cut('Right through aperture',.4,1.4,2.4,2.7,.18,FACE+.08)
...
box('Throat ceiling',-1,0,2.68,2.70,.05,FACE+.004,metal,.002)
box('Throat sill',-1,0,2.40,2.42,.18,FACE+.004,metal,.002)
for name,a,b in [('Left',-1,-.98),('Right',-.02,0)]:
 box(name+' throat return',a,b,2.42,2.68,.05,FACE+.004,metal,.002)
profile('Sloped internal receiving reveal',-1.02,.02,[(2.15,.02),(2.18,.02),(2.42,.18),(2.40,.18)],dark,.002)
...
flap=box('ChuteFlap',-.97,-.03,2.435,2.665,FACE-.030,FACE-.018,metal,.003)
bpy.context.scene.cursor.location=(-.5,-(FACE-.024),2.665); flap.select_set(True); bpy.context.view_layer.objects.active=flap; bpy.ops.object.origin_set(type='ORIGIN_CURSOR'); flap.select_set(False)
...
for x in [-1.49425,1.49425]:
 for y in [3.085-(2.24/8.7)*.4,3.085+(2.24/8.7)*.4]:
  box('Acrylic label standoff',x-.014,x+.014,y-.014,y+.014,FACE,FACE+.007,metal,.002)
...
scene['dimensions_ft']='3.2 W x 3.85 H x 1.823 D; rear -1.49, face .333; mouths (+/-.9,2.55,.333)'
scene['moving_parts']='ChuteFlap: top pivot (-.9,2.665,.309), opens inward during tape return; right independent flap at +.9'
for o in parts:
