...
// Source identifiers are opaque in Emby (they are NOT item identifiers). Keep
// playback metadata scoped by server and credential, including concurrent users.
const playbackSources = new Map<string, string>();
const playbackSessions = new Map<string, { PlaySessionId: string; MediaSourceId?: string; PlayMethod: string }>();
function playbackKey(server: string, token: string, itemId: string): string {
  return JSON.stringify([normalizeUrl(server), token, itemId]);
}
function selectedSource(server: string, token: string, itemId: string, explicit?: string): string | undefined {
  return explicit ?? playbackSources.get(playbackKey(server, token, itemId)) ?? (dialect === 'jellyfin' ? itemId : undefined);
}
function playbackFields(server: string, token: string, itemId: string) {
  if (dialect !== 'emby') return {};
  const key = playbackKey(server, token, itemId);
  if (!playbackSessions.has(key)) playbackSessions.set(key, {
    PlaySessionId: newPlaybackSessionId(), MediaSourceId: selectedSource(server, token, itemId), PlayMethod: 'DirectPlay',
  });
  const current = playbackSessions.get(key)!;
  current.MediaSourceId ??= selectedSource(server, token, itemId);
  return current;
}
...
function extractPlaybackInfo(item: any, server: string, token: string): MediaPlaybackInfo | undefined {
  if (item.MediaSources?.[0]?.Id) playbackSources.set(playbackKey(server, token, item.Id), item.MediaSources[0].Id);
 ...
 * (issue #125).
 */
async function validateToken(jellyfinUrl: string, token: string): Promise<boolean> {
  if (!jellyfinUrl || !token) return false;
  try {
    const url = normalizeUrl(jellyfinUrl);
    await jellyfinRequest("GET", `${url}/${dialect === "emby" ? "System/Info" : "Users/Me"}`, undefined, token);
    return true;
  } catch (e) {
...
const DEVICE_ID_KEY = `${dialect}_device_id`;
...
function clientIdentity(): string {
  return `${dialect === "emby" ? "Emby" : "MediaBrowser"} Client="Halcyon Video", Device="HTPC", DeviceId="${jellyfinDeviceId()}", Version="0.1.0"`;
}
...
async function jellyfinRequest(
  ...
  if (hasTauri) {
    ...
    return await invoke<string>("jellyfin_request", {
      method,
      url,
      authHeader: dialect === "emby" ? (authHeader || clientIdentity()) : buildAuthorization(authHeader, token),
      token: dialect === "emby" ? token : undefined,
      dialect,
      body
    });
  } else {
    ...
    const authorization = dialect === "emby" ? (authHeader || clientIdentity()) : buildAuthorization(authHeader, token);
    if (authorization) {
      headers[dialect === "emby" ? "X-Emby-Authorization" : "Authorization"] = authorization;
      if (dialect === "emby" && token) headers["X-Emby-Token"] = token;
    }
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(`HTTP error ${response.status}: ${dialect === "emby" ? "Media server request failed" : text}`);
      }
      return await response.text();
    } catch (e: any) {
      clearTimeout(timeoutId);
      if (e.name === 'AbortError') {
        throw new Error("Media server request timed out after 60 seconds");
      }
      throw e;
    }
  }
...
async function authenticateUser(
  ...
  try {
    ...
    if (!authData.AccessToken || !authData.User || !authData.User.Id) {
      throw new Error("Invalid response payload from media server.");
    }
    ...
    throw new Error(msg || "Failed to authenticate with media server.");
  }
}
 ...
 */
async function fetchPublicUsers(jellyfinUrl: string): Promise<PublicUser[]> {
  const url = normalizeUrl(jellyfinUrl);
 ...
 */
function collapseDuplicateVersions(movies: Movie[], context: string): Movie[] {
  ...
  if (collapsed > 0) {
    console.info(`[Media server] ${context}: collapsed ${collapsed} duplicate quality version(s) into their main title.`);
  }
  return out;
}
...
function filterCatalogItems(items: any[], context: string): any[] {
  ...
  if (skipped.size > 0) {
    ...
    console.info(`[Media server] ${context}: skipped ${items.length - kept.length} item(s) by type: ${parts}`);
  }
  return kept;
}
...
async function fetchItemsPaged(itemsUrl: string, token: string): Promise<any[]> {
  ...
  for (;;) {
    ...
    if (!Array.isArray(pageData.Items)) throw new Error("Media server returned an invalid items page");
    const pageItems: any[] = pageData.Items;
    for (const item of pageItems) items.push(item);
    pageProgressTick?.();

    const total = typeof pageData.TotalRecordCount === "number" ? pageData.TotalRecordCount : Infinity;
    // Empty-page guard keeps a server that under-reports pages from spinning forever.
    if (pageItems.length === 0 || items.length >= total || (total === Infinity && pageItems.length < CATALOG_PAGE_SIZE)) break;
  }
  return items;
}
...
async function fetchMediaCatalog(
  ...
  const url = normalizeUrl(jellyfinUrl);
  console.log(`[Media server] Attempting to connect to ${url} for user ${userId}...`);
  ...
  try {
    ...
    const movies: Movie[] = items.map((item: any) => {
      ...
      return {
        ...
        mediaPlaybackInfo: extractPlaybackInfo(item, url, token),
    ...
    console.log(`[Media server] Successfully synced ${collapsedMovies.length} titles.`);
    return { movies: collapsedMovies };
  } catch (error: any) {
    console.error("[Media server] Failed to sync metadata:", error);
 ...
 */
const collectionArt = new Map<string, { posterUrl?: string; backdropUrl?: string }>();
 ...
 * gaps.
 */
const collectionTmdbIds = new Map<string, number>();
 ...
 */
const collectionSyncStats = { boxSets: 0, scraped: 0, rejectedVersionPairs: 0 };
...
async function fetchCollectionMembership(
  ...
  if (rejected > 0) {
    console.log(`[Media server] Ignored ${rejected} single-title BoxSet(s) (version pairs, not real collections).`);
  }
...
async function applyCollectionMembership(
  ...
  try {
    ...
    console.log(`[Media server] Tagged ${tagged} titles from ${membership.size} collection memberships.`);
  } catch (err) {
    console.warn("[Media server] Collection (BoxSet) sync failed — shelves stay alphabetical:", err);
  }
}
...
async function fetchVideoViews(url: string, token: string, userId: string): Promise<any[]> {
  ...
  return viewItems.filter((v: any) => {
    ...
    console.info(
      `[Media server] View "${v.Name}" (Type=${v.Type}, CollectionType=${v.CollectionType ?? "none"}): ${keep ? "syncing" : "skipped (non-video)"}`
    );
    return keep;
  });
}
...
function knownServerLibraries(): LibrarySummary[] {
  if (typeof localStorage === 'undefined') return [];
  try {
...
async function fetchJellyfinLibrariesAndMovies(
  jellyfinUrl: string,
  token: string,
  userId: string,
  ...
  const url = normalizeUrl(jellyfinUrl);
  console.log(`[Media server] Querying user views (libraries) for user ${userId}...`);
  ...
  try {
    ...
    const movieLibraries = excluded?.size
      ? allVideoViews.filter((v: any) => {
          ...
          if (skip) {
            console.info(
              `[Media server] View "${v.Name}": skipped (not carried by this store — Settings → Connection → Store Libraries)`
            );
          }
          return !skip;
        })
    ...
    if (librariesList.length === 0 && allVideoViews.length > 0 && movieLibraries.length === 0) return [];
    if (librariesList.length === 0) {
      // Fallback: fetch all media recursively if views are empty or not filtering properly
      console.log("[Media server] No distinct movie libraries found. Falling back to global catalog sync.");
    ...
    onProgress?.('done');
    console.log(`[Media server] Successfully mapped ${librariesList.length} libraries.`);
    return librariesList;
  } catch (error: any) {
    console.error("[Media server] Failed to sync libraries and metadata:", error);
    const msg = error?.message ?? (typeof error === 'string' ? error : String(error));
    throw new Error(msg || "Failed to sync libraries from media server.");
  } finally {
...
async function fetchFirstEpisodeOfSeries(
  ...
  try {
    const responseStr = await jellyfinRequest(
      ...
      `${url}/Users/${userId}/Items?ParentId=${seriesId}&IncludeItemTypes=Episode&Recursive=true${dialect === "emby" ? "&Fields=Path,MediaSources" : ""}&SortBy=ParentIndexNumber,IndexNumber&SortOrder=Ascending&Limit=1`,
      undefined,
      token
    );
    ...
    if (item) {
      if (dialect === "emby") extractPlaybackInfo(item, url, token);
      return {
        id: item.Id,
        path: item.Path || ""
      };
    ...
    console.error(`[Media server] Failed to fetch first episode for series ${seriesId}:`, e);
  }
  return null;
}
...
async function fetchSeriesEpisodes(
  ...
  try {
    const responseStr = await jellyfinRequest(
      ...
      `${url}/Users/${userId}/Items?ParentId=${seriesId}&IncludeItemTypes=Episode&Recursive=true&Fields=Path,Overview,RunTimeTicks,PremiereDate,UserData${dialect === "emby" ? ",MediaSources" : ""}&SortBy=ParentIndexNumber,IndexNumber&SortOrder=Ascending&Limit=500`,
      undefined,
      token
    );
    const data = JSON.parse(responseStr);
    let items: any[] = data.Items || [];
    if (dialect === "emby") for (const item of items) extractPlaybackInfo(item, url, token);
    ...
    if (dialect === "emby") throw e;
    console.error(`[Media server] Failed to fetch episodes for series ${seriesId}:`, e);
    return [];
  }
}
...
async function reportPlaybackStart(
  ...
  try {
    await jellyfinRequest(
      ...
      clientIdentity(),
      token,
      JSON.stringify({ ...playbackFields(url, token, itemId), ItemId: itemId, CanSeek: false, IsPaused: false, IsMuted: false })
    );
    console.log(`[Media server] Playback start reported for item ${itemId}`);
  } catch (e) {
    if (dialect === "emby") throw e;
    console.warn("[Media server] Failed to report playback start:", e);
  }
}
...
async function reportPlaybackStopped(
  ...
  try {
    await jellyfinRequest(
      ...
      clientIdentity(),
      token,
      JSON.stringify({ ...playbackFields(url, token, itemId), ItemId: itemId, PositionTicks: positionTicks })
    );
    playbackSessions.delete(playbackKey(url, token, itemId));
    console.log(`[Media server] Playback stopped reported for item ${itemId} at tick ${positionTicks}`);
  } catch (e) {
    if (dialect === "emby") throw e;
    console.warn("[Media server] Failed to report playback stopped:", e);
  }
}
...
async function reportPlaybackProgress(
  ...
  try {
    await jellyfinRequest(
      ...
      clientIdentity(),
      token,
      JSON.stringify({ ...playbackFields(url, token, itemId), ItemId: itemId, PositionTicks: positionTicks, IsPaused: isPaused, CanSeek: true })
    );
  } catch (e) {
    if (dialect === "emby") throw e;
    console.warn("[Media server] Failed to report playback progress:", e);
  }
}
 ...
 * caller.
 */
async function stopActiveEncoding(
  ...
  if (dialect === "emby" && !conn) throw new Error("Emby transcode cancellation requires its source connection");
  ...
  try {
    ...
    console.warn("[Media server] Failed to stop active encoding:", e);
...
async function fetchItemPlaybackInfo(
  ...
  try {
    ...
    return extractPlaybackInfo(item, url, token);
  } catch (e) {
    console.error(`[Media server] Failed to probe media info for item ${itemId}:`, e);
    return undefined;
  }
}
 ...
 */
function buildStaticStreamUrl(jellyfinUrl: string, token: string, itemId: string, mediaSourceId?: string): string {
  const url = normalizeUrl(jellyfinUrl);
...
function isTextSubtitleCodec(codec: string | undefined): boolean {
 ...
 */
function buildSubtitleTrackUrl(
  jellyfinUrl: string,
  token: string,
  itemId: string,
  streamIndex: number,
...
function getLastHlsPlaySessionId(): string | undefined {
 ...
 */
function buildHlsStreamUrl(jellyfinUrl: string, token: string, itemId: string, opts?: HlsStreamOptions): string {
  const url = normalizeUrl(jellyfinUrl);
  const playSessionId = newPlaybackSessionId();
  lastHlsPlaySessionId = playSessionId;
  if (dialect === "emby") playbackSessions.set(playbackKey(url, token, itemId), { PlaySessionId: playSessionId, MediaSourceId: selectedSource(url, token, itemId, opts?.mediaSourceId), PlayMethod: "Transcode" });
...
function displayPreferencesUrl(jellyfinUrl: string, userId: string): string {
  const base = normalizeUrl(jellyfinUrl);
 ...
 */
async function fetchUserConfigPrefs(
  jellyfinUrl: string,
  token: string,
  userId: string
 ...
 */
async function saveUserConfigPrefs(
  jellyfinUrl: string,
  token: string,
  userId: string,
  ...
  for (const [k, v] of Object.entries(prefs)) {
    if (v.length > MAX_PREF_VALUE_LEN) {
      console.warn(
        `[Media server] Store config: "${k}" is ${v.length} chars, past the ${MAX_PREF_VALUE_LEN} ` +
