...
export function exitReturnLayout(storeWidth: number,vest: ExitGeometry): Footprint | null {
  ...
  const sideDepth=vest.frontZ-(vest.sideDoorZ+vest.doorW/2)-.2;
  const depth=sideDepth+7.1;
...
export function exitReturnSegments(f: Footprint): Footprint[] {
  const sx=f.w/15.5, sz=f.d/11.5, back=f.cz+f.d/2;
  ...
  return [segment([7.75,-3.2],[7.75,0],.8,'vestibule-stub'),
    ...
    segment([-7.75,0],[-7.75,-5.5],.8,'left'),
    segment([-7.75,-5.5],[-2,-11.5],.8,'outer-angle'),
    segment([-2,-11.5],[3.25,-6.25],.8,'front-angle'),
    segment([3.5,-.8],[-6.95,-.8],1.4,'window-worktop'),
    segment([-6.95,-5.2],[-2,-10.3],1.3,'inner-worktop-left'),
    segment([-2,-10.3],[2.4,-5.75],1.3,'inner-worktop-right')];
}
