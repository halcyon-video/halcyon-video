...
export function buildConeCanopyFallback(
  ...
  for (let row=0; row<3; row++) {
    ...
    box('Slate front panel',-w,w,lo,hi,front-.1,front,'FacadeSlate');
    for (const s of [-1,1]) box('Slate return panel',s<0?-w:w-.1,s<0?-w+.1:w,lo,hi,.1,front-.1,'FacadeSlate');
  }
  box('Coping cap',-w-.06,w+.06,top-.15,top,.02,front+.06,'FacadeCoping');
  box('Soffit',-w,w,bottom,bottom+.12,.1,front,'FacadeSoffit');
  for (const s of [-1,1]) {
    add('Inverted cone and tiered capital', new THREE.LatheGeometry(profile.pillarProfile.map(([r,y])=>new THREE.Vector2(r,y)),48).translate(s*profile.pillarX,0,profile.pillarZ),'FacadeSlate');
