...
test('exit counter fits small and large chain rooms, preserving a wall-side staff aisle',()=>{
  for(const width of [40,48,64,80])for(const doorW of [3,3.2,4]){
    const vest={xL:6,frontZ:15,sideDoorZ:vestibuleLayout({doorWidth:doorW,entryStyle:'vestibule'}).sideDoorZ,doorW,hasChamber:true};
...
test('enclosed return station leaves a usable staff entrance and sorting aisle',()=>{
  for(const width of [40,48,64,80]) {
    const f=exitReturnLayout(width,{xL:6,frontZ:15,sideDoorZ:vestibuleLayout({doorWidth:3.2,entryStyle:'vestibule'}).sideDoorZ,doorW:3.2,hasChamber:true})!;
