...
An original enclosed returns station shares the main checkout's blue/theme-colored rim, routed millwork profile, contrasting inlay and white lower worktops. The rear run sits against the front glass. A shortened customer-facing run leaves an entrance beside the vestibule; the open vestibule-side end admits staff, with a continuous closed wall at the far end. A plain return receiver stands on the window-side worktop. Its dimensions are design proportions, not measurements of a photographed store.
...
- Envelope: 15.5 by 7.4 feet; rim height 3.54 feet, white worktop height 2.82 feet. Vestibule-side staff approach 5 feet wide in the unscaled model, between the front run end and the vestibule-side datum. Six material batches, see the generated metrics for triangle count, closed manifold construction parts, UVs and outward normals.
...
Validation: production build and complete test suite, plus staff-entry path coverage. In-app context, overhead and interior evidence lives in `scratch/publicity-kits/vestibule-return-opening/` for direct dev landing. Private historical reference imagery is not included.

The vestibule-side edge is open: no wall runs flush along the vestibule. Only the rear wall returns to the frame beside the front glass. The front run ends before the vestibule so staff can enter from the store floor without crossing its glass. Vestibule depth and checkout datums are shared in `src/vestibule-layout.ts`; the default chamber grows 2.4 feet inward. Gates fit inside the enlarged chamber.
