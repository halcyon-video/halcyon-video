...
Each directory under `public/lighting/` contains little-endian RGBA half-float pixels in Three.js CubeUV PMREM layout, with dimensions and display intensity in its adjacent JSON file. The four maps are precomputed daytime environments from the 1990, 1993, 2000 and 2010 stores, not live ray-tracing effects or per-surface lightmaps. Mobile selects the map matching the active era. Layout-specific baked floor and wall contact occlusion, material surface maps and static shadow maps still provide grounding. Night lighting retains its existing environment path.
